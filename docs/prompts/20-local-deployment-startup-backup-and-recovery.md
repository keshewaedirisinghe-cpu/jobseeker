# Prompt 20 - Local Deployment Startup Backup And Recovery

## Execution order

Run this prompt as fresh context step **20**. After this prompt is complete, committed, and the prompt index is marked done by the agent, continue to the next prompt sequentially.
...
- `docs/milestones/20-local-deployment-startup-backup-and-recovery.md`
...
Implement only milestone **20**: One-command local deployment and recovery.

Dependencies recorded in the master index: **17-19**.
...
10. If checks pass, update required project records, commit with `milestone-20: <result>`, update the prompt index, and continue to the next prompt sequentially unless this is prompt 22.
