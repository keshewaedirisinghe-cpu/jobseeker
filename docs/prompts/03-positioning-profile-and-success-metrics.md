...
Run this prompt as fresh context step **03**. Do not run a later prompt until this prompt is complete, committed, and the prompt index is marked done by a human reviewer.
...
10. If checks pass, update required project records, commit with `milestone-03: <result>`, and stop before starting the next prompt.
