# Prompt 16 - Workers Scheduling Idempotency And Reliability

## Execution order

Run this prompt as fresh context step **16**. After this prompt is complete, committed, and the prompt index is marked done by the agent, continue to the next prompt sequentially.
...
- `docs/milestones/16-workers-scheduling-idempotency-and-reliability.md`
...
Implement only milestone **16**: Continuous local operation without duplicate work.

Dependencies recorded in the master index: **08-15**.
...
10. If checks pass, update required project records, commit with `milestone-16: <result>`, update the prompt index, and continue to the next prompt sequentially unless this is prompt 22.
